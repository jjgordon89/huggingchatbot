...
import { processDocumentFile } from '@/lib/api';
import { addDocumentToVectorStore, deleteDocumentFromVectorStore } from '@/lib/lanceDbService';
...
export const DocumentManager: React.FC<DocumentManagerProps> = ({ workspaceId }) => {
  ...
  useEffect(() => {
    if (workspaceId && !isVectorInitialized) {
      try {
        ...
        console.log('LanceDB vector store initialized for workspace:', workspaceId);
      } catch (error) {
        console.error('Failed to initialize LanceDB vector store:', error);
        toast({
          title: "Vector Store Error",
          description: "Failed to initialize document vector storage. Document search may not work properly.",
          variant: "destructive",
        });
      }
    }
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (indexingEnabled && isVectorInitialized) {
            try {
              // Process the file to extract its content
              const processedDoc = await processDocumentFile(file);
              
              const document = {
                id: docId,
                title: file.name,
                content: processedDoc.text,
              ...
              console.error(`Error adding document to vector store: ${file.name}`, error);
            }
          }
  ...
  const getFileIcon = (fileName: string) => {
    ...
    if (['pdf'].includes(extension || '')) return <Badge variant="outline" className="bg-red-500/10">PDF</Badge>;
    if (['doc', 'docx'].includes(extension || '')) return <Badge variant="outline" className="bg-blue-500/10">DOC</Badge>;
    if (['xls', 'xlsx', 'csv'].includes(extension || '')) return <Badge variant="outline" className="bg-green-500/10">CSV</Badge>;
    if (['jpg', 'jpeg', 'png', 'gif'].includes(extension || '')) return <Badge variant="outline" className="bg-purple-500/10">IMG</Badge>;
    if (['txt', 'md'].includes(extension || '')) return <Badge variant="outline" className="bg-gray-500/10">TXT</Badge>;
    
    return <Badge variant="outline"><FileType className="h-3 w-3" /></Badge>;
  };
        ...
        <div className="flex gap-2">
          ...
          <div className="relative">
            <Input
              ...
              disabled={uploading}
            />
            <Button variant="outline" className="flex gap-1.5 items-center" disabled={uploading}>
          ...
          {filteredDocuments.length > 0 ? (
            <div className="space-y-2">
              {filteredDocuments.map(doc => (
                <div key={doc.id} className="flex items-center justify-between p-2 rounded-md border group hover:bg-muted/50">
            ...
            <div className="flex flex-col items-center justify-center h-[200px] text-center text-muted-foreground">
              ...
              <p className="text-sm opacity-70 max-w-[300px] mt-1">
                Upload documents to enable RAG capabilities for more accurate and contextually relevant responses
              </p>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor="indexing-toggle" className="text-xs cursor-pointer">
                      Vector Indexing
                    </Label>
                  </div>
                </TooltipTrigger>
                <TooltipContent side="top">
                  <p className="text-xs max-w-[200px]">
                    When enabled, documents will be processed and indexed in the vector database for AI retrieval
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
            ...
            <Button
              ...
              onClick={async () => {
                toast({
                  ...
                  description: "Updating vector database with the latest document content...",
                });
                ...
                try {
                  ...
                  toast({
                    ...
                    description: "An error occurred while updating the vector database.",
                    variant: "destructive",
                  });
                }
              }}
