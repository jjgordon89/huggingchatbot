...
export function ChatMessages() {
  ...
  const collapseOlderMessages = () => {
    ...
    messages.slice(0, Math.max(0, messages.length - messagesToKeep))
        ...
        {messages.length === 0 ? (
          <div className="text-center py-12 px-4">
            ...
            <p className="text-muted-foreground mb-8 max-w-md mx-auto">
              Start a conversation with your AI assistant powered by {activeModel?.name || 'advanced AI'}.
              Ask questions, brainstorm ideas, or get help with various tasks.
            </p>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-md mx-auto">
              <SamplePrompt 
                text="Explain quantum computing in simple terms" 
                icon={<ArrowUpRight className="h-3.5 w-3.5 text-cyber-primary" />}
              />
              <SamplePrompt 
          ...
          messagesWithContext.map((message) => {
            ...
            return (
              <div
                ...
                onClick={() => collapsedMessages.has(message.id) && toggleMessageCollapse(message.id)}
              >
                {isAssistant ? (
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        "font-medium text-sm",
                        isAssistant ? "text-cyber-primary" : "text-foreground"
                      )}>
                  ...
                  {collapsedMessages.has(message.id) ? (
                    ...
                    <div className={cn(
                      ...
                      isAssistant ? "prose-headings:text-primary/90" : "prose-headings:text-foreground"
                    )}>
                      <ReactMarkdown>
                        {message.content}
                      </ReactMarkdown>
          ...
          <div className="flex items-start gap-3 rounded-lg bg-card/60 p-4 border border-border/10 shadow-sm">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge variant="outline" className="h-5 animate-pulse bg-primary/5 border-primary/10">
...
function SamplePrompt({ text, icon }: { text: string; icon?: React.ReactNode }) {
  const { sendMessage } = useChat();
  
  return (
    <button 
      onClick={() => sendMessage(text)}
      className="group p-3 text-left text-sm bg-muted/40 hover:bg-muted/70 border border-border/30 hover:border-cyber-primary/30 rounded-lg transition-all duration-200 relative overflow-hidden"
    >
      <div className="absolute inset-0 bg-gradient-to-r from-cyber-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      <div className="flex justify-between items-center">
        <span className="relative z-10 font-medium">{text}</span>
        {icon && <span className="relative z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200">{icon}</span>}
      </div>
    </button>
  );
}
