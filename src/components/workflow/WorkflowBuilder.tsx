...
import { WorkflowNode as WorkflowNodeType, NodeType, WorkflowEdge } from '@/lib/workflowTypes';
...
import { ChevronLeft, Save, PlayCircle, LayoutGrid, AlertCircle, CheckCircle, Loader2 } from 'lucide-react';
...
export const WorkflowBuilder: React.FC<WorkflowBuilderProps> = ({ workflowId, onBack }) => {
  const {
    getWorkflow,
    createWorkflow,
    updateWorkflow,
  ...
  // Workflow state
  const [workflow, setWorkflow] = useState(workflowId ? getWorkflow(workflowId) : null);
  ...
  useEffect(() => {
    if (workflowId) {
      ...
      if (existingWorkflow) {
        ...
        const flowNodes = existingWorkflow.nodes.map((node: WorkflowNodeType) => ({
          id: node.id,
          type: node.type,
  ...
  }, [workflow, workflowName, workflowDescription, nodes, edges, updateWorkflow, createWorkflow, addNode, addWorkflowEdge]);
  ...
  const createFromTemplate = useCallback((templateId: string) => {
    const newWorkflow = createWorkflow(workflowName, workflowDescription, templateId);
    setWorkflow(newWorkflow);
  ...
  }, [workflowName, workflowDescription, createWorkflow, getWorkflow, setNodes, setEdges]);
  ...
  const handleRunWorkflow = useCallback(() => {
    ...
    import('@/lib/workflowExecutionService').then(({ workflowExecutionService }) => {
          ...
          setNodes(prevNodes => 
            prevNodes.map(node => ({
              ...
              data: {
                ...
                errorMessage: node.id === nodeId ? error : node.data.errorMessage
              }
            }))
          );
        },
  ...
  }, [workflow, saveWorkflow, setNodes, toast, setExecutionResults, setExecutionLogs, setShowExecutionResults, setIsExecuting]);
  ...
  return (
    <div className="flex flex-col h-full">
                ...
                <Card className="cursor-pointer hover:border-primary transition-colors border-blue-200">
                  <CardHeader className="pb-2 bg-blue-50">
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-lg">OpenRouter AI Agent</CardTitle>
                      <Badge variant="outline" className="bg-blue-100">New</Badge>
                    </div>
                    <CardDescription>Powerful agent using OpenRouter models</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="text-sm text-muted-foreground">
                      An AI agent workflow leveraging OpenRouter's 100+ models with RAG capabilities, web search, and more.
                    </div>
                  </CardContent>
                  <CardFooter>
                    <Button
                      className="w-full"
                      onClick={() => createFromTemplate('openrouter-agent-template')}
                    >
                      Use Template
                    </Button>
                  </CardFooter>
            ...
            <TabsContent value="empty" className="space-y-4 mt-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Create a workflow from scratch</CardDescription>
                </CardHeader>
      ...
      <Dialog open={showExecutionResults} onOpenChange={setShowExecutionResults}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              Workflow Execution {executionResults?.success ? (
                <Badge variant="outline" className="ml-2 bg-green-50 text-green-700 border-green-200">Successful</Badge>
              ) : (
                <Badge variant="outline" className="ml-2 bg-red-50 text-red-700 border-red-200">Failed</Badge>
              )}
            </DialogTitle>
          </DialogHeader>
            ...
            <TabsContent value="results" className="space-y-4 mt-4">
              {executionResults?.success ? (
                <div>
                  ...
                  {executionResults?.output && Object.keys(executionResults.output).length > 0 ? (
                    <div className="bg-slate-50 p-4 rounded-md overflow-auto max-h-80">
                      <pre className="text-sm">{JSON.stringify(executionResults.output, null, 2)}</pre>
                    </div>
                  ) : (
                ...
                <div className="bg-red-50 p-4 rounded-md border border-red-200">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-medium text-red-700 mb-1">Execution Failed</h3>
                      <p className="text-red-600">{executionResults?.error || 'Unknown error occurred'}</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-md">
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm">{nodes.filter(n => n.data.isCompleted).length} completed</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <AlertCircle className="h-4 w-4 text-red-500" />
                        <span className="text-sm">{nodes.filter(n => n.data.isError).length} failed</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-4 rounded-md">
                    ...
                    <p className="text-sm mt-2">{executionResults?.executionTime || '---'}</p>
                  </div>
                </div>
              </div>
            </TabsContent>
          ...
          <div className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setShowExecutionResults(false)}>Close</Button>
