...
const groupNodesBySubcategory = (nodes: NodeTypeItem[], category: string): GroupedNodes => {
  return nodes
...
export const NodePalette: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (storedFavorites) {
        ...
        localStorage.setItem('workflow-favorite-nodes', JSON.stringify(defaultFavorites));
      }
  ...
  const toggleFavorite = (nodeType: string) => {
    ...
    localStorage.setItem('workflow-favorite-nodes', JSON.stringify(newFavorites));
  };
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium flex items-center">
                  {node.name}
                  {node.isNew && <Badge className="ml-2 py-0 h-4 text-[9px] bg-green-100 text-green-800">New</Badge>}
                  {node.isExperimental && <Badge className="ml-2 py-0 h-4 text-[9px] bg-amber-100 text-amber-800">Experimental</Badge>}
                </div>
  ...
  const renderCategory = (categoryName: string, displayName: string, icon: React.ReactNode) => {
    const categoryNodes = filteredNodes.filter(node => node.category === categoryName);
    ...
    return (
      ...
      >
        <CollapsibleTrigger 
          className="flex items-center justify-between w-full py-2 px-3 rounded-md hover:bg-accent/10 font-medium"
        >
          <div className="flex items-center gap-2">
            {icon}
            <span>{displayName}</span>
            <Badge className="ml-2 bg-muted text-muted-foreground">{categoryNodes.length}</Badge>
          </div>
          <ChevronRightIcon className={`h-4 w-4 transition-transform ${openCategories[categoryName] ? 'rotate-90' : ''}`} />
        </CollapsibleTrigger>
      ...
      case 'categories':
        return (
          <div className="space-y-1">
            {renderCategory('flow', 'Flow Control', <SplitIcon className="h-4 w-4 text-amber-500" />)}
            {renderCategory('ai', 'AI & ML', <BrainIcon className="h-4 w-4 text-purple-500" />)}
            {renderCategory('data', 'Data Processing', <DatabaseIcon className="h-4 w-4 text-blue-500" />)}
            {renderCategory('programming', 'Programming', <CodeIcon className="h-4 w-4 text-indigo-500" />)}
            {renderCategory('integrations', 'Integrations', <ArrowRightFromLineIcon className="h-4 w-4 text-teal-500" />)}
            {renderCategory('visualization', 'Visualization', <BarChartIcon className="h-4 w-4 text-blue-500" />)}
          </div>
        );
          ...
          <div className="space-y-2">
            {favoriteNodes.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">Click the star on a node to add it to favorites</p>
              </div>
            )}
          </div>
        );
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b bg-slate-50/40">
        ...
        <p className="text-sm text-muted-foreground">Drag and drop nodes to create your workflow</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        ...
        <ScrollArea className="flex-1 px-4 py-2">
          <TabsContent value={activeTab} className="mt-0 data-[state=active]:flex data-[state=active]:flex-col">
            {renderTabContent()}
          </TabsContent>
        </ScrollArea>
      </Tabs>
