...
const CodeBlock = ({ node, inline, className, children, ...props }: any) => {
  ...
  return !inline && match ? (
    <SyntaxHighlighter
      ...
      language={match[1]}
      PreTag="div"
      className="rounded-md border border-primary/20 !my-3"
      {...props}
    >
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    message.isParentMessage && "border-l-3 border-cyber-primary shadow-[0_0_15px_rgba(249,115,22,0.15)]"
                  )}
                >
                  ...
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      isAssistant ? "prose-h1:text-cyber-primary prose-h2:text-cyber-primary/90 prose-h3:text-cyber-primary/80" : ""
                    )}>
                      <ReactMarkdown
                        components={{
                          code: CodeBlock
...
function SamplePrompt({ text, icon }: { text: string; icon?: React.ReactNode }) {
  ...
  return (
    <button
      ...
      className="group p-4 text-left text-sm bg-muted/60 hover:bg-muted/90
                border-2 border-border/40 hover:border-cyber-primary/50
                rounded-xl shadow-sm hover:shadow-md
                transition-all duration-300 relative overflow-hidden
                hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-cyber-primary/30"
    >
      ...
      <div className="flex justify-between items-center gap-3">
        ...
        {icon && (
          <span className="relative z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200
                        p-1.5 bg-cyber-primary/10 rounded-full">
            {icon}
          </span>
        )}
      </div>
