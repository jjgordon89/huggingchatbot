...
export function configureWebSearch(options: {
  ...
  return {
     ...
     */
    search: async (query: string, options?: WebSearchOptions): Promise<SearchResult[]> => {
      ...
      const mockResults: SearchResult[] = [
        {
          ...
          snippet: 'Transformers provides APIs to easily download and use those pretrained models on a given text, fine-tune them on your own datasets...',
        ...
        },
        {
          ...
          snippet: 'Semantic search seeks to improve search accuracy by understanding the content of the search query instead of using keyword matching.',
          source: 'sbert.net',
        ...
        },
        {
          ...
          snippet: 'Vector embeddings are numerical representations that capture the semantics or meaning of data in a way that machines can understand and process.',
          source: 'pinecone.io',
...
export function getPreferredSearchEngine(): SearchEngine {
  ...
  if (preference && ['brave', 'google', 'duckduckgo', 'serpapi'].includes(preference)) {
...
export async function searchWeb(
  ...
  const mockResults: SearchResult[] = [
    ...
    },
    {
      ...
      source: 'example.com',
      publishedDate: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    },
    {
      ...
      source: 'example.com',
      publishedDate: new Date(Date.now() - 21 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
    }
  ];
